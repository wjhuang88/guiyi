# Documentation index

## Approved reference

- [Project charter](reference/PROJECT-CHARTER.md)
- [Architecture](reference/ARCHITECTURE.md)
- [AI-native platform](reference/AI-NATIVE-PLATFORM.md)
- [Command and query protocol](reference/COMMAND-QUERY-PROTOCOL.md)
- [Stage model](reference/STAGE-MODEL.md)
- [Lessons extracted from wuxia](reference/WUXIA-EXTRACTION-LESSONS.md)

## Delivery

- [Implementation roadmap](roadmap/IMPLEMENTATION-ROADMAP.md)
- [Product backlog](backlog/PRODUCT-BACKLOG.md)
- [Iteration 001 plan](iterations/iteration-001/PLAN.md)
- [Iteration 001 validation](iterations/iteration-001/VALIDATION.md)
- [Iteration 001 retrospective](iterations/iteration-001/RETROSPECTIVE.md)

## Standard operating procedures

- [Development SOP](sop/DEVELOPMENT.md)
- [Iteration SOP](sop/ITERATION.md)
- [Quality gates](sop/QUALITY-GATES.md)
- [AI agent operation SOP](sop/AGENT-OPERATIONS.md)
- [Release SOP](sop/RELEASE.md)

## Architecture decisions

See [the ADR register](decisions/README.md).

## Bootstrap and verification

- [Generated package report](bootstrap/PACKAGE-REPORT.md)
- [First local run](bootstrap/FIRST-RUN.md)
